...
/**
 *  应用初始化成功之后由CP调用此方法,此方法是整合平台的初始化方法
 *  @warning 此方法必须调用
 *  @note  just like this:
...
/**
 *  由CP在application:didRegisterForRemoteNotificationsWithDeviceToken:方法中调用
 *  @warning 必须接入此方法
 *  @param application UIApplication对象
...
/** 支付相关方法 **/
/**
 *  订单支付功能，如果order为空则弹出充值页面，如果该渠道不支持定额支付 则order将被忽略直接弹出充值页面
 *  @param order 订单信息
 */
-(void)paymentWithOrder:(ACOrderObject *)order;
-(void)showChargingView;                        //调出充值页面
...
-(void)accessChannel:(id<ACAccessChannel>)channel didFinishChargingWithOrder:(ACPaymentResult *)order error:(NSError *)error;//支付结束之后调用，如果支付失败error不为空
@end
...
ACKit_EXTERN NSString *const ACAccessChannelDidFinishCharginNotification;
